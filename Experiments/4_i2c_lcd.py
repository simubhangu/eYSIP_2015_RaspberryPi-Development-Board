...
def initialize_mcp():
    # all bits of IODIRA and IODIRB register are set to 0 meaning GPA and GPB pins are outputs
...
# Logic : Command 0x30 is sent thrice and command 0x20 is sent once to initialize the LCD 
...
# Output : The function returns a list with corresponding decimal equivalent of the hexadecimal values 
# Logic : A 2 digit hexadecimal value is seperated into 1 digit each and a zero is appended at the end 
#         of each digit(eg: '0f' is converted to '00' and 'f0')
...
# Output : The function returns a list with corresponding hexadecimal equivalent of every character in a string 
# Logic : This function converts a character in a string into hex format and appends the 
#         converted hex value into a list for further processing. (eg: 'A' is converted to '41' i.e. the hex value of the character )
# Example call: stringconvert(s)
def stringconvert(s):
    s1 = []
    l1 = len(s)
    for i in range(0,l1):
        j = ord(string[i]) # ord() function returns the decimal equivalent of an ascii character
        f = hex(j)
        s1.append(f)
...
# Output : The function returns 2 lists 'data' and 'cmd'  that contain data and commands in the form 
#          that can be directly given as a pin output of MCP23017 IC 
...
# Input : A list with commands in converted form (hexadecimal to decimal format)
# Output : Commands are sent to LCD display one by one 
# Logic : For sending commands RS pin of an LCD is set to 0 and R/W pin of an LCD is set to 0(for write operation)
#         and a high to low enable pulse is applied every time a command is sent
...
# Logic : For sending data RS pin of an LCD is set to 1 and R/W pin of an LCD is set to 0(for write operation)
#         and a high to low enable pulse is applied every time data is sent
